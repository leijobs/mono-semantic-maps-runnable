...
from .pyramid import PyramidOccupancyNetwork, PyramidFusionOccupancyNetwork, PyramidFusionDetectionNetwork
...
from ..nn.pointpillar import PointPillarsScatter
from ..nn.centerpoint import CenterHead
from ..nn.topdown import TopdownNetwork, FusionTopdownNetwork
...
def build_model(model_name, config):
    if model_name == 'pyramid':
        ...
        model = build_pyramid_fusion_occupancy_network(config)
    elif model_name == 'detection':
        model = build_pyramid_fusion_detection_network(config)
    elif model_name == 'ved':
...
def build_pyramid_fusion_occupancy_network(config):
    # Build frontend
    frontend = FPN50()

    # Build point frontend
    point_frontend = PointPillarsScatter()
    ...
    # Build topdown network
    fusion_topdown = FusionTopdownNetwork(config.tfm_channels, config.topdown.channels,
                                          config.topdown.layers, config.topdown.strides,
                                          config.topdown.blocktype)

    # Build classifier
    if config.bayesian:
        classifier = BayesianClassifier(fusion_topdown.out_channels, config.num_class)
    else:
        classifier = LinearClassifier(fusion_topdown.out_channels, config.num_class)
    classifier.initialise(config.prior)

    # Assemble Pyramid Occupancy Network
    return PyramidFusionOccupancyNetwork(frontend, point_frontend, transformer, fusion_topdown, classifier)


def build_pyramid_fusion_detection_network(config):
    # Build frontend
    frontend = FPN50()
    ...
    # Build detector
    detector = CenterHead(fusion_topdown.out_channels, config.num_class)

    # Assemble Pyramid Occupancy Network
    return PyramidFusionDetectionNetwork(frontend, point_frontend, transformer, fusion_topdown, detector)
